      ...
      switch ($_GET["op"]) {
        ...
        case 'guardar_y_editar':
          ...
          if (empty($idexperiencia)){            
            ...
            if ($flat_img1 == true) {
              $datos_f1 = $experiencia->obtenerImg($idexperiencia);
              $img1_ant = $datos_f1['data']['img_perfil'];
              if ( !empty($img1_ant) ) { unlink("../dist/docs/experiencia/perfil/" . $img1_ant);  }
            }              

            // editamos un paquete existente
            $rspta=$experiencia->editar($idexperiencia, $nombre, $lugar, $comentario, $estrella,$imagen1);
            echo json_encode($rspta, true);
          }            
        ...
        case 'mostrar':
          $rspta=$experiencia->mostrar($_POST["idexperiencia"]);
          ...
          if ($rspta['status'] == true) {
            foreach ($rspta['data'] as $key => $value) {
              ...
              $data[]=array(
                ...
                <button class="btn btn-warning btn-sm" onclick="mostrar_experiencia(' . $value['idexperiencia'] . ')" data-toggle="tooltip" data-original-title="Desactivado"><i class="fas fa-pencil-alt"></i></button>
                <button class="btn btn-danger  btn-sm" onclick="eliminar_experiencia(' . $value['idexperiencia'] . ', \''.$value['nombre'].'\' )" data-toggle="tooltip" data-original-title="Eliminar o Papelera"><i class="fas fa-skull-crossbones"></i></button>',
