    ...
    if ($_SESSION['pedido'] == 1) {
      ...
      switch ($_GET["op"]) {
        ...
        case 'mostrar_detalle_tours':
          $rspta=$pedido->mostrar_tours($_POST["idpaquete"], $_POST["idpedido_paquete"]);
          //Codificar el resultado utilizando json
          echo json_encode($rspta, true);
        break;    
        
        case 'vendido':
          $rspta=$pedido->vendido_tours($_POST["idpedido_paquete"]);
          echo json_encode($rspta, true);
        break;
        ...
        case 'tbla_principal_tours':          
          ...
          if ($rspta['status'] == true) {
            ...
            foreach ($rspta['data'] as $key => $value) {        
              ...
              $data[]=array(
                "0"=>$cont++,
                "1"=>' <button class="btn btn-info btn-sm" onclick="mostrar_pedido(' . $value['idtours'] .', '.$value['idpedido_tours']. ')" data-toggle="tooltip" data-original-title="Ver"><i class="fa fa-eye"></i></button>' .
                ' <button class="btn btn-danger  btn-sm" onclick="eliminar_pedido(' . $value['idpedido_tours'] .')" data-toggle="tooltip" data-original-title="Eliminar o Papelera"><i class="fas fa-skull-crossbones"></i></button>'.
                ' <button class="btn btn-success  btn-sm" onclick="vendido(' . $value['idpedido_tours'] .')" data-toggle="tooltip" data-original-title="Vender"><i class="fa-solid fa-cart-shopping"></i></button>',
