...
function init() {
  ...
  $("#bloc_lGaleriaPaquete").addClass("menu-open bg-color-191f24");

  $("#mlGaleriaPaquete").addClass("active");

  $("#mlGaleriaPaquete").addClass("active bg-green");

  $("#lGaleriaPaquete").addClass("active");
  ...
  $("#guardar_registro_galeria_paquete").on("click", function (e) { $("#submit-form-galeria_paquete").submit(); });
...
function tbla_principal() {
  tabla_galeria_paquete = $("#tabla-galeria_paquete").dataTable({
    responsive: true,
    ...
    buttons: [      
      { text: '<i class="fa-solid fa-arrows-rotate" data-toggle="tooltip" data-original-title="Recargar"></i>', className: "btn bg-gradient-info", action: function ( e, dt, node, config ) { tabla_galeria_paquete.ajax.reload(null, false); toastr_success('Exito!!', 'Actualizando tabla', 400); } },
      { extend: 'copyHtml5', exportOptions: { columns: [0,1,3], }, text: `<i class="fas fa-copy" data-toggle="tooltip" data-original-title="Copiar"></i>`, className: "btn bg-gradient-gray", footer: true,  }, 
      { extend: 'excelHtml5', exportOptions: { columns: [0,1,3], }, text: `<i class="far fa-file-excel fa-lg" data-toggle="tooltip" data-original-title="Excel"></i>`, className: "btn bg-gradient-success", footer: true,  }, 
      { extend: 'pdfHtml5', exportOptions: { columns: [0,1,3], }, text: `<i class="far fa-file-pdf fa-lg" data-toggle="tooltip" data-original-title="PDF"></i>`, className: "btn bg-gradient-danger", footer: false, orientation: 'landscape', pageSize: 'LEGAL',  },
      { extend: "colvis", text: `Columnas`, className: "btn bg-gradient-gray", exportOptions: { columns: "th:not(:last-child)", }, },
    ],
    ajax: {
      url: "../ajax/galeria_paquete.php?op=tbla_principal",
      type: "get",
      dataType: "json",
      error: function (e) {
...
function guardar_y_editar_galeria_paquete(e) {
  // e.preventDefault(); //No se activará la acción predeterminada del evento
  var formData = new FormData($("#form-galeria-paquete")[0]);

  $.ajax({
    url: "../ajax/galeria_paquete.php?op=guardar_y_editar_galeria_paquete",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
    success: function (e) {
      try {
        ...
        if (e.status == true) {
          ...
          $('#modal-agregar-galeria_paquete').modal('hide'); //
          limpiar_form();    
...
function eliminar_galeria_paquete(idgaleria_paquete) {

  crud_eliminar_papelera(
    "../ajax/galeria_paquete.php?op=desactivar",
    "../ajax/galeria_paquete.php?op=eliminar", 
    idgaleria_paquete, 
    "!Elija una opción¡", 
    ...
    function(){ tabla_galeria_paquete.ajax.reload(null, false); },
    false, 
    false, 
    false,
    false
...
//Función limpiar
function limpiar_galeria_paquete() {
...
$(function () {   
  ...
  $("#form-galeria-paquete").validate({
    ...
    submitHandler: function (e) {
      guardar_y_editar_galeria_paquete(e);
    },
...
function ver_img_galeria_paquete(file, nombre) {
  ...
  $(".tooltip").removeClass("show").addClass("hidde");
  $("#modal-ver-imagen-galeria_paquete").modal("show");
  $('#imagen-galeria_paquete').html(`<span class="jq_image_zoom"><img class="img-thumbnail" src="${file}" onerror="this.src='../dist/svg/404-v2.svg';" alt="Perfil" width="100%"></span>`);
  $('.jq_image_zoom').zoom({ on:'grab' });
}