...
function agregarDetalleComprobante(idpaquete, individual) {
  ...
  if (idpaquete != "") {    
    ...
    if ($(`.producto_${idpaquete}`).hasClass("producto_selecionado") && individual == false ) {    
      if (document.getElementsByClassName(`producto_${idpaquete}`).length == 1) {
        var cant_producto = $(`.producto_${idpaquete}`).val();
        var sub_total = parseInt(cant_producto, 10) + 1;
        $(`.producto_${idpaquete}`).val(sub_total).trigger('change');
        toastr_success("Agregado!!",`Producto: ${$(`.nombre_producto_${idpaquete}`)[0].innerText} agregado !!`, 700);
        modificarSubtotales();      
      }      
    } else {         
      $.post("../ajax/ajax_general.php?op=mostrar_producto_paquete", {'idpaquete': idpaquete}, function (e, textStatus, jqXHR) {          
        ...
        if (e.status == true) {         
          ...
          var img_p = e.data.imagen == "" || e.data.imagen == null ?img_p = `../dist/docs/paquete/perfil/paquete-sin-foto.jpg` : `../dist/docs/paquete/perfil/${e.data.imagen}` ;          

          var fila = `
          <tr class="filas" id="fila${cont}">         
            <td class="py-1">
              <button type="button" class="btn btn-warning btn-sm" onclick="mostrar_productos(${e.data.idpaquete}, ${cont})"><i class="fas fa-pencil-alt"></i></button>
              <button type="button" class="btn btn-danger btn-sm btn-file-delete-${cont}" onclick="eliminarDetalle(${e.data.idpaquete}, ${cont});"><i class="fas fa-times"></i></button>
            </td>
            <td class="py-1">         
              <input type="hidden" name="idpaquete[]" value="${e.data.idpaquete}">
              <div class="user-block text-nowrap">
                ...
                <span class="username"><p class="mb-0 nombre_producto_${e.data.idpaquete}">${e.data.nombre}</p></span>
                <span class="description categoria_${cont}"><b>Dcto: </b> ${parseFloat(e.data.porcentaje_descuento)}%<b> |<b>Estado: </b>disponible</span>
              </div>
            </td>
            <td class="py-1">
              ...
              <input class="tipo_paquete_${cont}" type="hidden" name="tipo_paquete[]" id="tipo_paquete[]" value="${e.data.tipo_paquete}">
            </td>
            <td class="py-1 form-group">
              <input type="number" class="w-100px valid_cantidad form-control producto_${e.data.idpaquete} producto_selecionado" name="valid_cantidad[${cont}]" id="valid_cantidad_${cont}" value="${cantidad}" min="0.01" required onkeyup="replicar_value_input(${cont}, '#cantidad_${cont}', this); update_price(); " onchange="replicar_value_input(${cont}, '#cantidad_${cont}', this); update_price(); ">
          ...
          array_data_venta_paquete.push({ id_cont: cont });
          modificarSubtotales();        
...
function modificarSubtotales() {  
  ...
  var val_igv = $('#impuesto').val(); //console.log(array_data_venta_paquete);
  ...
  if ($("#tipo_comprobante").select2("val") == null) {    
    ...
    if (array_data_venta_paquete.length === 0) {
    } else {
      array_data_venta_paquete.forEach((element, index) => {
...
function eliminarDetalle(idpaquete, indice) {
  $("#fila" + indice).remove();
  array_data_venta_paquete.forEach(function (car, index, object) { if (car.id_cont === indice) { object.splice(index, 1); } });
  modificarSubtotales();
  detalles = detalles - 1;
  evaluar();
...
function mostrar_venta(idventa_producto) {
  ...
  $.post("../ajax/venta_paquete.php?op=ver_venta_editar", { idventa_producto: idventa_producto }, function (e, status) {
    ...
    if (e.status == true) {      
      ...
      if (e.data.detalle) {
        ...
        e.data.detalle.forEach((val, index) => {
          ...
          <tr class="filas" id="fila${cont}">
            <td>
              <button type="button" class="btn btn-warning btn-sm" onclick="mostrar_productos(${val.idpaquete}, ${cont})"><i class="fas fa-pencil-alt"></i></button>
              <button type="button" class="btn btn-danger btn-sm btn-file-delete-${cont}" onclick="eliminarDetalle(${val.idpaquete}, ${cont});"><i class="fas fa-times"></i></button></td>
            </td>
            <td>
              <input type="hidden" name="idpaquete[]" value="${val.idpaquete}">
              <div class="user-block text-nowrap">
            ...
            <td class="py-1 form-group">
              <input class="w-100px valid_cantidad form-control producto_${val.idpaquete} producto_selecionado" type="number" name="valid_cantidad[${cont}]" id="valid_cantidad_${cont}" value="${val.cantidad}" min="0.01" required onkeyup="replicar_value_input(${cont}, '#cantidad_${cont}', this);" onchange="replicar_value_input(${cont}, '#cantidad_${cont}', this);">
              <input type="hidden" class="cantidad_${cont}" name="cantidad[]" id="cantidad_${cont}" value="${val.cantidad}" onkeyup="update_stock(${val.idpaquete},${cont});" onchange="update_stock(${val.idpaquete},${cont});">              
    ...
    if (e.status == true) {
      ...
      if (e.data.detalle) {
        ...
        e.data.detalle.forEach((val, index) => {
          ...
          update_stock(val.idpaquete, cont);
...
function ver_detalle_ventas(idventa_producto) {
  ...
  $.post(`../ajax/ajax_general.php?op=ver_detalle_ventas&idventa_producto=${idventa_producto}`, function (e) {
    ...
    if (e.status == true) {
      ...
      $("#excel_compra").removeClass('disabled').attr('href', `../reportes/export_xlsx_venta_paquete.php?id=${idventa_producto}`);
