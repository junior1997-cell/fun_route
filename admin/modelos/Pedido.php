<?php
  ...
  class Pedido
  {
    ...
    public function mostrar($idpaquete)
    {
      ...
      $sql="SELECT p.idpaquete, p.nombre, p.duracion, p.descripcion, p.imagen FROM paquete as p WHERE p.idpaquete='$idpaquete';";
      $datospaquete = ejecutarConsultaSimpleFila($sql); if ($datospaquete['status'] == false) { return  $datospaquete;}

      $sql_1="SELECT i.iditinerario, i.mapa, i.incluye, i.no_incluye,i.recomendaciones FROM itinerario as i WHERE i.idpaquete='$idpaquete';";
      $datositinerario = ejecutarConsultaSimpleFila($sql_1); if ($datositinerario['status'] == false) { return  $datositinerario;}

      $sql_2="SELECT gp.idgaleria_paquete, gp.imagen,gp.descripcion FROM galeria_paquete as gp WHERE gp.idpaquete='$idpaquete';";
      $datosgaleria = ejecutarConsultaArray($sql_2); if ($datosgaleria['status'] == false) { return  $datosgaleria;}
    ...
    public function tbla_principal()
    {
      ...
      p.nombre as paquete, p.duracion, p.descripcion , p.imagen as imgpaquete 
